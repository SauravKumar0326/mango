...
public class FilterParser {
	...
	final static Logger logger = Logger.getLogger(FilterParser.class);
	/**
	...
	class Node {
		 ...
		 *
		 * @param pos
		 *            The position of the node in the filter string.
		 */
	...
	class OperatorNode extends Node {
		 ...
		 *
		 * @param filterType
		 *            The type of operator, either SCIMFilterType.AND or
		 *            SCIMFilterType.OR.
		 * @param pos
		 *            The position of the node in the filter string.
		 */
	...
	class LeftParenthesisNode extends Node {
		 ...
		 *
		 * @param pos
		 *            The position of the parenthesis in the filter string.
		 */
	 ...
	 */
	public SCIMFilter parse() throws MangoException {
		try {
			return readFilter();
		} catch (Exception e) {
			logger.error(
					"Error parsing search filter" + "Invalid filter " + filterString + " message " + e.getMessage());
			e.printStackTrace();
	...
	private SCIMFilter readFilterComponent() {
		...
		try {
			...
			logger.error("Error parsing attribute " + e);
		...
		return new SCIMFilter(filterType, filterAttribute, filterValue != null ? filterValue.toString() : null,
	...
	private SCIMFilter readFilter() {
		...
		// Employ the shunting-yard algorithm to parse into reverse polish
		// notation,
		...
		// Evaluate the reverse polish notation to create a single complex
		// filter.
		...
		if (filterString.charAt(currentPos) == '"') {
			...
			loop: while (currentPos < endPos) {
				...
				case '`':
					...
					// because
					// they are included in the range of values between '-' and
					// 'z', and making sure all possible characters are included
					// can help
					// make the switch statement more efficient. We'll fall
			...
			Object value = s;// null;
			/*
			 * try { value = JsonUtils.fromJson(s, Object.class); } catch
			 * (Exception e) { throw new IllegalArgumentException(msg); } if
			 * (value == null || value instanceof String) { throw new
